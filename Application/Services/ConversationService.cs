...
public class ConversationService
{
    ...
    public ConversationService(IConfiguration configuration)
    {
        ...
        _conversations = database.GetCollection<Conversation>(configuration["MongoDbSettings:DbHistory"]);
    }
    ...
    public async Task AddMessageToConversationAsync(string userId, string? conversationId, string userMessage,
        string aiResponse)
    {
        ...
        if (string.IsNullOrEmpty(conversationId))
        {
            ...
            Console.WriteLine(
                $"Nova conversa criada para {userId} com título '{conversation.Title}'. ID: {conversation.Id}");
        }
        else
        {
            ...
            {
                Console.WriteLine(
                    $"Aviso: ConversationId {conversationId} não encontrado para o usuário {userId}. Criando nova conversa.");
                conversation = new Conversation
                {
                    UserId = userId,
                    Title = userMessage.Length > 50 ? userMessage.Substring(0, 50) + "..." : userMessage
