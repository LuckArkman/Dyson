...
public class CpuMathEngine : IMathEngine
    {
        ...
        public void CloneTo(IMathTensor source, IMathTensor destination)
        {
            ...
            var d = ((CpuTensor)destination).GetData();
            
            if (s.Length != d.Length)
                throw new ArgumentException("Tamanhos incompatíveis.");
                
            Array.Copy(s, d, s.Length);
        }
        
        // ✅ IMPLEMENTAÇÃO DO FILL
        ...
        // --- MÉTODOS DE BATCH (Fallback CPU) ---

        public void LookupBatch(IMathTensor embeddingMatrix, int[] indices, IMathTensor result)
        {
            // Fallback simples iterando sobre o batch
            int batchSize = indices.Length;
            int embeddingSize = embeddingMatrix.Shape[1];
            var embData = ((CpuTensor)embeddingMatrix).GetData();
            var resData = ((CpuTensor)result).GetData();

            for (int i = 0; i < batchSize; i++)
            {
                int tokenIndex = indices[i];
                int srcOffset = tokenIndex * embeddingSize;
                int dstOffset = i * embeddingSize;
                Array.Copy(embData, srcOffset, resData, dstOffset, embeddingSize);
            }
        }
        ...
        public void AccumulateGradientBatch(IMathTensor embeddingGradients, IMathTensor batchGradients, int[] indices)
        {
            ...
            var embGradData = ((CpuTensor)embeddingGradients).GetData();
            var batchGradData = ((CpuTensor)batchGradients).GetData();

            // Nota: Em CPU single-thread não precisamos de atomics, mas em multi-thread precisaria de lock.
            // Assumindo execução sequencial simples aqui para o fallback.
            for (int i = 0; i < batchSize; i++)
            {
                int tokenIndex = indices[i];
                int gradOffset = i * embeddingSize;
                int embOffset = tokenIndex * embeddingSize;

                for (int j = 0; j < embeddingSize; j++)
                {
                    embGradData[embOffset + j] += batchGradData[gradOffset + j];
                }
            }
        }
        ...
        {
            var data = ((CpuTensor)resultTensor).GetData();
            ...
            {
                int targetIndex = indices[i];
                if (targetIndex >= 0 && targetIndex < vocabSize)
                {
                    data[i * vocabSize + targetIndex] = 1.0f;
                }
            }
        }
            ...
            float lr, float beta1, float beta2, float epsilon, int t)
        {
            ...
            for (int i = 0; i < p_data.Length; i++)
            {
                ...
                // Sanitize input gradient locally
                if (float.IsNaN(grad) || float.IsInfinity(grad)) grad = 0.0f;
                grad = Math.Clamp(grad, -10.0f, 10.0f);

                m_data[i] = beta1 * m_data[i] + (1 - beta1) * grad;
                v_data[i] = beta2 * v_data[i] + (1 - beta2) * (grad * grad);
                
                float m_hat = m_data[i] / (1 - beta1_pow_t);
                float v_hat = v_data[i] / (1 - beta2_pow_t);
                
                float update = lr * m_hat / (MathF.Sqrt(v_hat) + epsilon);
                
                // Sanitize update
                if (float.IsNaN(update) || float.IsInfinity(update)) update = 0.0f;
                update = Math.Clamp(update, -0.1f, 0.1f);
