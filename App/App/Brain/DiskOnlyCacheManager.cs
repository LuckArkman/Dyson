...
public class DiskOnlyCacheManager : IDisposable
    {
        ...
        public DiskOnlyCacheManager(int embeddingSize, int hiddenSize)
        {
        ...
        /// </summary>
        /// <summary>
        /// ✅ MÉTODO REATORADO: Recupera múltiplos conjuntos de dados brutos (float[] e shape) de um timestep.
        /// Não cria mais tensores, eliminando a dependência da IMathEngine.
        /// </summary>
        public Dictionary<string, (float[] Data, int[] Shape)> RetrieveMultipleTensors(int timeStep, params string[] tensorNames)
        {
            var results = new Dictionary<string, (float[] Data, int[] Shape)>();
            ...
            foreach (var name in tensorNames)
            {
                ...
                // Lê os dados brutos do arquivo.
                long length = reader.ReadInt64();
                var dataBytes = reader.ReadBytes((int)(length * sizeof(float))); // Note: tamanho em bytes
                var floatData = MemoryMarshal.Cast<byte, float>(dataBytes).ToArray();

                results[name] = (floatData, shape);
            }
        ...
        /// </summary>
        public (float[] Data, int[] Shape) RetrieveTensor(int timeStep, string tensorName)
        {
            var result = RetrieveMultipleTensors(timeStep, tensorName);
            if (result.TryGetValue(tensorName, out var tensorData))
                return tensorData;
