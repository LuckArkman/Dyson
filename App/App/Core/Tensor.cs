...
public class Tensor : IMathTensor
{
    ...
    public void ReadFromStream(BinaryReader reader, long length)
    {
        if (length != this.Length)
        {
            throw new InvalidDataException(
                $"Inconsistência no tamanho do tensor. Esperado pelo objeto: {this.Length}, informado pelo gerenciador: {length}.");
        }
        ...
        try
        {
            ...
            if (totalBytesRead != bytesToRead)
            {
                throw new EndOfStreamException(
                    $"Não foi possível ler a quantidade esperada de bytes ({bytesToRead}) do stream. Bytes lidos: {totalBytesRead}.");
            }
    ...
    public void Dispose()
    {
        /* Nenhum recurso não gerenciado para liberar. */
    }
}